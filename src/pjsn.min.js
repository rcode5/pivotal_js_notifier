var j=jQuery;var pjsn={showNote:function(content){top.trackerRef=window.open('','mytracker','width=800,height=800'
+',menubar=0'
+',toolbar=1'
+',status=0'
+',scrollbars=1'
+',resizable=1')
top.trackerRef.document.writeln('<html><head>'
...
+'</body></html>')
top.consoleRef.document.close()},init:function(){var _that=this;var getStoryName=function(startB){var desc=j(startB).closest('.storyItem').find('.story_name')[0];return(desc.textContent||desc.innerText).replace(/(.*)\s+\(.*/,"$1");};j('#layout').on('mousedown','a.start',function(){var story=getStoryName(this);_that.showNote(story);});this.init=function(){};}};jQuery(document).ready(pjsn.init());
